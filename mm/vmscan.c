...
static inline int zone_is_near_oom(struct mem_cgroup_zone *mz)
{
	...
	return mz->zone->pages_scanned >= (anon + file) * 3;
}
			       ...
			       int priority, int file)
{
	...
	if (vm_enable_legacy_mm && sc->may_swap && scanning_global_lru(mz)) {
		long mapped_ratio;
		long distress;
		long swap_tendency;

		if (zone_is_near_oom(mz))
...
force_reclaim_mapped:
	...
	while (!list_empty(&l_hold)) {
		...
		if (vm_enable_legacy_mm && page_mapped(page) &&
		    page_is_file_cache(page) && scanning_global_lru(mz)) {
			/*
			...
			if (!reclaim_mapped) {
				...
				continue;
			} else if (page_referenced(page, 0, sc->target_mem_cgroup,
						   &vm_flags)) {
