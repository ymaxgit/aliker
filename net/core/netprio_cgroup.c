...
static void free_priomap_rcu(struct rcu_head *head)
{
	struct netprio_map *map = container_of(head, struct netprio_map, rcu);
...
static void extend_netdev_table(struct net_device *dev, u32 new_len)
{
	...
	old_priomap  = rcu_dereference(data->priomap);
